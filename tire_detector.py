...
#ap.add_argument("-m", "--model", required=True, help="yolo4' or 'yolo4tiny'")
...
def draw_predictions(image):
    ...
    #caption_text = 'Model: {}'.format(model_name)
    caption_text = ' No objects detected'
    if len(idxs) > 0:  # ensure at least one detection exists
        ...
        caption_text = " Inference time: {:.1f} ms".format(infer_time * 1000)
    # Caption
...
if args["input"] == 'image':
    ...
    # python tire_detector.py -f video1.mp4 -i video
    ...
    while True:
        ...
        if writer is None:
            ...
            writer = cv2.VideoWriter(os.path.sep.join(["Output", args["file"]]),
